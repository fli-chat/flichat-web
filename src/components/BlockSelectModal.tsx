interface BlockSelectModalProps {
  onClickBlockSuccess: () => void;
  onClose: () => void;
}

export default function BlockSelectModal({ onClickBlockSuccess, onClose }: BlockSelectModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-semantic-teriary rounded-[10px] w-[295px] mx-[20px]">
        ...
        <div className="flex flex-col justify-center items-center px-[32px] py-[22px] pb-[16px] ">
          ...
          <h3 className="title4 font-bold text-font-primary">차단하시겠습니까?</h3>
        </div>
        ...
        <div className="flex gap-[7px] justify-between items-center px-[32px] py-[16px]">
          ...
          <button className="w-full h-[42px] rounded-[8px] body1 font-medium transition-colors duration-200 bg-semantic-accent text-semantic-primary" onClick={onClickBlockSuccess}>확인</button>
        </div>
      </div>
    </div>
  );
